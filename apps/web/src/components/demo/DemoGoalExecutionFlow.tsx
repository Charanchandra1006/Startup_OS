...
/**
 * DemoGoalExecutionFlow — Scripted pitch demo animation.
 *
 * Extracted from the original GoalExecutionFlow.tsx so the demo path
 * is never reachable from a logged-in founder's normal goal-submission
 * flow. Only accessible via explicit /demo route or ?demo=true.
 *
 * STARTUP_OS_MASTER_BUILD_PLAN Part 3.5: "Demo/Real Split"
 */
...
export function DemoGoalExecutionFlow({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    // Uses the unauthenticated /demo/series-a SSE endpoint (line 209 in server.js)
  ...
  if (state === "starting") {
    return (
      <div className="p-8 text-center bg-white border border-slate-200 rounded-2xl animate-pulse text-indigo-600 shadow-sm">
        Initializing Chief OS Demo Dispatcher...
      </div>
    );
  }
